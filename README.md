...
| **Options** | Connection, refresh interval, inactivity threshold, About |
...
mecm-health-dashboard/
  ...
  Logs/                             Session and crash logs (auto-created)
...
- All SQL and WMI queries use unlimited timeouts (`-QueryTimeout 0`, `-OperationTimeoutSec 0`) to support large environments on slow links. If queries still fail, check network connectivity to the SQL / WMI server.
...
- On connection, the dashboard sets `Set-CMQueryResultMaximum -Maximum 0` (unlimited) to ensure all deployments are returned. If rows are still missing, verify the CM console user account has permission to view all deployments.
